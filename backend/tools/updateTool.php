...
if ($id <= 0)                                         { http_response_code(400); echo json_encode(['error'=>'Valid tool ID required']); exit; }
...
try {
    $stmt = $pdo->prepare("
        UPDATE tools SET
            year=?, tool_name=?, type=?, no_of_license=?,
            job_slots=?, resume_views=?, bulk_mail=?,
    ...
    ");
    $stmt->execute([
        $year,$toolName,$type,$noOfLicense,
        $jobSlots,$resumeViews,$bulkMail,
    ...
    if ($stmt->rowCount() === 0) { http_response_code(404); echo json_encode(['error'=>'Tool not found']); exit; }
    ...
    http_response_code(500); echo json_encode(['error'=>'Failed to update tool']);
}